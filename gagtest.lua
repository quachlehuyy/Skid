...
-- Hàm auto loop đa năng, dùng cho mọi auto
local function RunAutoLoop(flagName, option, action, delay, extraCond)
    if AutoLoopFlags[flagName] then return end
    AutoLoopFlags[flagName] = true
    task.spawn(function()
        while option.Value and (not extraCond or extraCond()) do
            action()
            task.wait(delay or 0.1)
        end
        AutoLoopFlags[flagName] = false
    end)
end
...
Tabs.Shop:AddToggle("AutoBuySeed", {Title="Auto Buy Seed", Default=config.AutoBuySeed}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoBuySeed", Options.AutoBuySeed, function()
...
Tabs.Shop:AddToggle("AutoBuyAllSeed", {Title="Auto Buy All Seed", Default=config.AutoBuyAllSeed}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoBuyAllSeed", Options.AutoBuyAllSeed, buyallseed, 0.05)
    end
end)
...
Tabs.Shop:AddToggle("AutoBuyGear", {Title="Auto Buy Gear", Default=config.AutoBuyGear}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoBuyGear", Options.AutoBuyGear, function()
...
Tabs.Shop:AddToggle("AutoBuyAllGear", {Title="Auto Buy All Gear", Default=config.AutoBuyAllGear}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoBuyAllGear", Options.AutoBuyAllGear, buyallgear, 0.05)
    end
end)
...
Tabs.Shop:AddToggle("AutoBuyEgg", {Title="Auto Buy Egg", Default=config.AutoBuyEgg}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoBuyEgg", Options.AutoBuyEgg, function()
...
Tabs.Shop:AddToggle("AutoBuyAllEgg", {Title="Auto Buy All Egg", Default=config.AutoBuyAllEgg}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoBuyAllEgg", Options.AutoBuyAllEgg, buyallegg, 0.05)
    end
end)
...
Tabs.Shop:AddToggle("AutoBuyItem", {Title="Auto Buy Item", Default=config.AutoBuyItem}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoBuyItem", Options.AutoBuyItem, function()
...
Tabs.Shop:AddToggle("OpenGoliathShop", {Title="Open Goliath Shop", Default=config.OpenGoliathShop}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("OpenGoliathShop", Options.OpenGoliathShop, function()
...
Tabs.Farm:AddToggle("AutoPlant", {Title="Auto Plant", Default=config.AutoPlant}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoPlant", Options.AutoPlant, function()
...
Tabs.Farm:AddToggle("AutoHarvest", {Title="Auto Harvest", Default=config.AutoHarvest}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoHarvest", Options.AutoHarvest, function()
...
Tabs.Farm:AddToggle("AutoSellFruit", {Title="Auto Sell Fruit", Default=config.AutoSellFruit}):OnChanged(function(Value)
    ...
    SaveConfig()
    if Value then
        RunAutoLoop("AutoSellFruit", Options.AutoSellFruit, function()
...
Tabs.Farm:AddToggle("AutoSellInventory", {Title="Auto Sell Inventory", Default=config.AutoSellInventory}):OnChanged(function()
    ...
    SaveConfig()
    if Options.AutoSellInventory.Value then
        RunAutoLoop("AutoSellInventory", Options.AutoSellInventory, sellallinventory, delaySellValue)
    end
end)
